...
loss_function = ME(device=device)
...
def train(epoch):
    ...
    if args.setting == 'reweight':
        ...
        loss_train = loss_function(output[idx_train_new], labels_new[idx_train_new], weight=weight)
        ...
        loss_train = loss_function(output[idx_train_new], labels_new[idx_train_new])
...
# encoder.eval()
# embed = encoder(features, adj)
# np.save('latent_space.npy', embed.detach().cpu().numpy())
